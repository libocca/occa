...
int main(int argc, const char** argv)
{
	...
	p.n1 = 256;   // First dimension
  	...
  	p.n3_Tblock=1;       // Thread blocking on 3rd dimension
...
# define N3_TBLOCK 124 // Default thread blocking on 3rd dimension: 124
 	...
 	if(p.n1%p.n1_Tblock != 0){
		std::cerr<<"The size on first dimension must be a multiple of the blocking size on the first dimension"<<std::endl;
		return -1;
	}	
	...
	for(int i=1; i<= HALF_LENGTH; i++){
  	...
  	// allocate dat memory
  	size_t nsize = (p.n1+16)*p.n2*p.n3;
	...
	int entries = (p.n1 +16)* p.n2 * p.n3;
  	...
  	kernelProps["defines/WGSX"] = p.n1_Tblock;//WGSX=p.n1_Tblock
	...
	for(int it=0; it<p.nreps; it+=2){
		iso3dfdkernel(o_next, o_prev, o_vel, o_coeff, p.n1, p.n2, p.n3, 8);
		iso3dfdkernel(o_prev, o_next, o_vel, o_coeff, p.n1, p.n2, p.n3, 8);
  	} // time loop
	...
	o_prev2.copyFrom(p.prev);
        ...
        //time iteration loop
	for(int i=0; i<2; i++){
        	iso3dfdkernel(o_next2, o_prev2, o_vel, o_coeff, p.n1, p.n2, p.n3, 8);
        	iso3dfdkernel(o_prev2, o_next2, o_vel, o_coeff, p.n1, p.n2, p.n3, 8);
	}
	o_prev2.copyTo(p.prev);
        o_next2.copyTo(p.next);


        float *p_ref = (float*)malloc((p.n1+2*HALF_LENGTH)*p.n2*p.n3*sizeof(float));
        if( p_ref==NULL ){
...
*/
       if( within_epsilon( p.next, p_ref, p.n1, p.n2, p.n3, HALF_LENGTH, 0, 1e-38f ) ) {
